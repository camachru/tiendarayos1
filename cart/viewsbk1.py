...
class PostalView(generic.TemplateView):
    template_name = 'cart/cpostal.html'

    def post(self, request, *args, **kwargs):       
        context = self.get_context_data(**kwargs)
                
        seleccion = self.request.POST.get('postal')
        
        datcod = get_object_or_404(DjangoPostalcodesMexicoPostalcode, d_codigo=seleccion)

        context["codigoa"]  = datcod
        print(datcod)
