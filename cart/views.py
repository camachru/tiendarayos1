...
class PostalView(generic.FormView):
    ...
    def get_success_url(self):
        ...
        return reverse('cart:checkout3', kwargs={'cpostal': self.cpostal})

    def form_valid(self, form):
        self.cpostal=form.cleaned_data['d_codigo']
        
        
        return super(PostalView, self).form_valid(form)
...
class CheckoutView2(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        order = get_or_set_order_session(self.request)
        selected_colonia1 = form.cleaned_data.get('selected_colonia')
        

                 
               
        address = Address.objects.create(calle = form.cleaned_data.get('calle'),       
        address_type    = 'S',
        user            = self.request.user,
        zip_code        = self.get_object(),
        no_ext          =        form.cleaned_data.get('no_ext'),
        no_int          =        form.cleaned_data.get('no_int'),
        telefono        =      form.cleaned_data.get('telefono'),
        Indicaciones    =  form.cleaned_data.get('Indicacionesa'),
        tipo_dom    =  form.cleaned_data.get('tipo_dom'),
        mnpiociu        = selected_colonia1
    ...
    def get_form_kwargs(self):
        ...
        kwargs["user_id"] = self.request.user.id 
        kwargs["cpostalk1"] = self.get_object() 
                    
        # kwargs["cpostalk1"]  = self.kwargs["dcodigo1"]
