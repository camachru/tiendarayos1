...
class ValCpostalForm(forms.Form):
    cpostal = forms.CharField(required=True, label="Códico Postal",widget=forms.TextInput(attrs={'placeholder': 'Código Postal'}))
    

    def clean(self):
        cpostal1 = self.cleaned_data['d_codigo']
        cpostal2 = DjangoPostalcodesMexicoPostalcode.objects.get(d_codigo=self.cpostal1)
        

        if not cpostal1:
            raise forms.ValidationError(f"Código Postal no existe  {cpostal2.cpostal1}")
